...
df = pd.DataFrame(columns = ['Type','Contour Id','Perimeter','Eccentricity','Extent','Solidity','BLue', 'Green', 'Red', 'Point Polygon Test'])
print(df)
...
for i in range(1,len(files) + 1) :
	...
	for cnt in contours :
		if cv2.contourArea(cnt) > 1100 :
			...
			df.loc[len(df)] = [c, i, cv2.arcLength(cnt,True), ecc, extent, solidity, mean_val[0], mean_val[1], mean_val[2], dist]
print(df)
